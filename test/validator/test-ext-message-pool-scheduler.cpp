...
TEST(ExtMessagePoolScheduler, CallbackExcludedPrefixBootstrapTraversesDisjointRangesForOneSource) {
  auto pool = ExtMessagePoolTestAccess::make_pool();
  auto source = ExtMessagePoolTestAccess::source(39);
  ExtMessagePoolTestAccess::set_watermark(pool, source, 0);
  std::vector<ExtMessage::Hash> exclusions;
  for (td::uint64 nonce = 0; nonce <= 10; ++nonce) {
    auto hash = ExtMessagePoolTestAccess::add(pool, source, nonce);
    if (nonce <= 2 || (nonce >= 6 && nonce <= 8)) {
      exclusions.push_back(hash);
    }
  }

  // The scheduler must skip both [0, 2] and [6, 8] synchronously while
  // retaining the unexcluded messages between and after those ranges.
  ExtMessagePoolTestAccess::install_live_waiting_callback(pool, 5, 5, exclusions);
  ASSERT_EQ(ExtMessagePoolTestAccess::bootstrap_callback_prefill(pool), 5u);
  ASSERT_EQ(ExtMessagePoolTestAccess::callback_pending_native_locations(pool),
            (std::vector<std::pair<ExtMessagePoolTestAccess::NativeAddress, td::uint64>>{
                {source, 3}, {source, 4}, {source, 5}, {source, 9}, {source, 10}}));

  auto stats = ExtMessagePoolTestAccess::scheduler_stats(pool);
  ASSERT_EQ(stats.selected, 5u);
  ASSERT_EQ(stats.excluded, 6u);
  ASSERT_EQ(stats.excluded_prefix_index_builds, 1u);
  ASSERT_EQ(stats.excluded_prefix_index_entries, 6u);
  ASSERT_EQ(stats.excluded_prefix_index_ranges, 2u);
  ASSERT_EQ(stats.excluded_prefix_shortcut_messages, 6u);
  ASSERT_EQ(stats.excluded_prefix_fallbacks, 0u);
}

TEST(ExtMessagePoolScheduler, CallbackExcludedPrefixBootstrapStopsAtTerminalNonce) {
  auto pool = ExtMessagePoolTestAccess::make_pool();
  auto source = ExtMessagePoolTestAccess::source(40);
  const auto terminal_nonce = std::numeric_limits<td::uint64>::max();
  const auto first_nonce = terminal_nonce - 1;
  ExtMessagePoolTestAccess::set_watermark(pool, source, first_nonce);
  auto first_hash = ExtMessagePoolTestAccess::add(pool, source, first_nonce);
  auto terminal_hash = ExtMessagePoolTestAccess::add(pool, source, terminal_nonce);

  // The final inclusive range [UINT64_MAX - 1, UINT64_MAX] must count both
  // exclusions and stop, never wrap the nonce back to zero.
  ExtMessagePoolTestAccess::install_live_waiting_callback(pool, 4, 4, {first_hash, terminal_hash});
  ASSERT_EQ(ExtMessagePoolTestAccess::bootstrap_callback_prefill(pool), 0u);
  ASSERT_EQ(ExtMessagePoolTestAccess::callback_pending_native_locations(pool).size(), 0u);
  auto stats = ExtMessagePoolTestAccess::scheduler_stats(pool);
  ASSERT_EQ(stats.scanned, 2u);
  ASSERT_EQ(stats.excluded, 2u);
  ASSERT_EQ(stats.excluded_prefix_index_builds, 1u);
  ASSERT_EQ(stats.excluded_prefix_index_entries, 2u);
  ASSERT_EQ(stats.excluded_prefix_index_ranges, 1u);
  ASSERT_EQ(stats.excluded_prefix_shortcut_messages, 2u);
  ASSERT_EQ(ExtMessagePoolTestAccess::first_unconsumed_nonce(pool, source), first_nonce);

  // A fresh losing-branch callback can still select exactly the terminal
  // prefix; this catches both unwanted canonical mutation and nonce wrap.
  ExtMessagePoolTestAccess::discard_live_callback(pool);
  ExtMessagePoolTestAccess::install_live_waiting_callback(pool, 4, 4);
  ASSERT_EQ(ExtMessagePoolTestAccess::prefill_native_transport(pool), 2u);
  ASSERT_EQ(ExtMessagePoolTestAccess::callback_pending_native_locations(pool),
            (std::vector<std::pair<ExtMessagePoolTestAccess::NativeAddress, td::uint64>>{
                {source, first_nonce}, {source, terminal_nonce}}));
  ASSERT_EQ(ExtMessagePoolTestAccess::first_unconsumed_nonce(pool, source), first_nonce);
}
