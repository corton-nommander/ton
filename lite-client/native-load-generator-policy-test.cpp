...
TEST(NativeLoadGeneratorPolicy, PacingClippingUsesOverflowMassAcrossPhaseBoundaries) {
  native_load::PacingTelemetry stats;
  native_load::MeasureWindow phase{10.0, 12.0};
  // Warmup interval: the bucket fills at 9.2, before measurement starts.
  // Only the 50 tokens earned in [10,10.5) are measured loss, not 130/3.
  stats.observe_refill(9.0, 10.5, 80.0, 100.0, 100.0, 100.0, phase);
  ASSERT_TRUE(std::abs(stats.clipped_tokens - 130.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_clipped_tokens - 50.0) < 1e-9);
  ASSERT_EQ(stats.clipped_updates, 1u);
  ASSERT_EQ(stats.measure_clipped_updates, 1u);
  ASSERT_EQ(stats.measure_start.samples, 1u);
  ASSERT_TRUE(std::abs(stats.measure_start.refilled_tokens - 100.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_start.max_service_lag_seconds - 0.5) < 1e-9);
  // End interval: credit becomes capped only at 12.5, entirely during drain.
  // The exact end balance is 50, even though the post-drain gauge is 100.
  stats.observe_refill(11.5, 13.0, 0.0, 100.0, 100.0, 100.0, phase);
  ASSERT_TRUE(std::abs(stats.clipped_tokens - 180.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_clipped_tokens - 50.0) < 1e-9);
  ASSERT_EQ(stats.clipped_updates, 2u);
  ASSERT_EQ(stats.measure_clipped_updates, 1u);
  ASSERT_EQ(stats.measure_end.samples, 1u);
  ASSERT_TRUE(std::abs(stats.measure_end.refilled_tokens - 50.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_end.max_service_lag_seconds - 1.0) < 1e-9);
  stats.observe_refill(13.0, 14.0, 100.0, 100.0, 100.0, 100.0, phase);
  ASSERT_EQ(stats.measure_end.samples, 1u);
  ASSERT_TRUE(std::abs(stats.measure_end.refilled_tokens - 50.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_clipped_tokens - 50.0) < 1e-9);
}

TEST(NativeLoadGeneratorPolicy, PacingClippingPreservesEndpointRefillAndFractionalCredit) {
  native_load::PacingTelemetry stats;
  // Linear endpoint interpolation earns 50*t*t. At t=1 there is no overflow;
  // at t=1.5 exactly 12.5 has overflowed. Uniform time proration gives 25.
  stats.observe_refill(0.0, 2.0, 0.0, 0.0, 200.0, 100.0, {1.0, 1.5});
  ASSERT_TRUE(std::abs(stats.clipped_tokens - 100.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_clipped_tokens - 12.5) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_start.refilled_tokens - 50.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_end.refilled_tokens - 100.0) < 1e-9);

  native_load::PacingTelemetry fractional;
  // Two ticks with fractional carry can issue a full quantum. Observing
  // either interval never rounds or consumes the caller's pacing balance.
  const double first_balance = 9.5;
  fractional.observe_refill(0.0, 0.01, 0.0, 950.0, 950.0, 95.0, {0.0, 1.0});
  fractional.observe_refill(0.01, 0.02, first_balance, 950.0, 950.0, 95.0, {0.0, 1.0});
  ASSERT_TRUE(std::abs(first_balance - 9.5) < 1e-9);
  ASSERT_TRUE(std::abs(fractional.clipped_tokens) < 1e-9);
  ASSERT_EQ(fractional.clipped_updates, 0u);
}

TEST(NativeLoadGeneratorPolicy, PacingRefillRejectsInvalidAndZeroLengthIntervals) {
  native_load::PacingTelemetry stats;
  native_load::MeasureWindow phase{1.0, 2.0};
  auto nan = std::numeric_limits<double>::quiet_NaN();
  auto inf = std::numeric_limits<double>::infinity();
  stats.observe_refill(1.0, 1.0, 100.0, 100.0, 100.0, 100.0, phase);
  stats.observe_refill(2.0, 1.0, 100.0, 100.0, 100.0, 100.0, phase);
  stats.observe_refill(nan, 2.0, 100.0, 100.0, 100.0, 100.0, phase);
  stats.observe_refill(1.0, inf, 100.0, 100.0, 100.0, 100.0, phase);
  stats.observe_refill(1.0, 2.0, nan, 100.0, 100.0, 100.0, phase);
  stats.observe_refill(1.0, 2.0, 100.0, -1.0, 100.0, 100.0, phase);
  stats.observe_refill(1.0, 2.0, 100.0, 100.0, inf, 100.0, phase);
  stats.observe_refill(1.0, 2.0, 100.0, 100.0, 100.0, nan, phase);
  ASSERT_EQ(stats.clipped_updates, 0u);
  ASSERT_EQ(stats.measure_start.samples, 0u);
  ASSERT_TRUE(std::abs(stats.update_gap.all_seconds) < 1e-9);
  // An exact boundary update records the pre-issue balance only once.
  stats.observe_refill(0.0, 1.0, 0.0, 100.0, 100.0, 100.0, phase);
  stats.observe_refill(1.0, 2.0, 0.0, 100.0, 100.0, 100.0, phase);
  ASSERT_EQ(stats.measure_start.samples, 1u);
  ASSERT_EQ(stats.measure_end.samples, 1u);
  ASSERT_TRUE(std::abs(stats.measure_start.refilled_tokens - 100.0) < 1e-9);
  ASSERT_TRUE(std::abs(stats.measure_start.max_service_lag_seconds) < 1e-9);
  ASSERT_TRUE(std::abs(stats.clipped_tokens) < 1e-9);
}

TEST(NativeLoadGeneratorPolicy, TimingMaximaUseMeasuredIntersectionAndWorkerMaximum) {
  native_load::MeasureWindow phase{10.0, 12.0};
  native_load::ObservedIntervalMax first;
  first.observe(9.0, 13.0, phase);
  first.observe(11.0, 11.0, phase);
  first.observe(12.0, 11.0, phase);
  first.observe(11.0, std::numeric_limits<double>::infinity(), phase);
  ASSERT_TRUE(std::abs(first.all_seconds - 4.0) < 1e-9);
  ASSERT_TRUE(std::abs(first.measure_seconds - 2.0) < 1e-9);
  native_load::ObservedIntervalMax other;
  other.observe(12.0, 20.0, phase);
  first.merge(other);
  ASSERT_TRUE(std::abs(first.all_seconds - 8.0) < 1e-9);
  ASSERT_TRUE(std::abs(first.measure_seconds - 2.0) < 1e-9);
  ASSERT_TRUE(std::abs(phase.overlap(8.0, 10.0)) < 1e-9);
  ASSERT_TRUE(std::abs(phase.overlap(12.0, 14.0)) < 1e-9);
  ASSERT_TRUE(std::abs(native_load::MeasureWindow{12.0, 10.0}.overlap(9.0, 13.0)) < 1e-9);
}

TEST(NativeLoadGeneratorPolicy, ClientIssueWaitSnapshotsPreserveOpenIntervalAndExactPhase) {
  native_load::IssueWaitTelemetry wait;
  native_load::MeasureWindow phase{10.0, 12.0};
  wait.update(true, 9.0, phase);
  wait.update(true, 9.5, phase);  // Repeated pumps must not restart the interval.
  auto before = wait.snapshot(10.0, phase);
  ASSERT_TRUE(std::abs(before.seconds - 1.0) < 1e-9);
  ASSERT_EQ(before.events, 1u);
  ASSERT_EQ(before.measure_events, 0u);
  auto open = wait.snapshot(11.0, phase);
  auto repeated = wait.snapshot(11.0, phase);
  ASSERT_TRUE(std::abs(open.seconds - 2.0) < 1e-9);
  ASSERT_TRUE(std::abs(open.measure_seconds - 1.0) < 1e-9);
  ASSERT_TRUE(std::abs(repeated.measure_seconds - open.measure_seconds) < 1e-9);
  ASSERT_EQ(open.measure_events, 1u);
  wait.update(false, 11.5, phase);  // A response frees one whole quantum.
  ASSERT_TRUE(!wait.active());
  wait.update(false, 11.5, phase);  // Repeated callback close is harmless.
  auto closed = wait.snapshot(20.0, phase);
  ASSERT_TRUE(std::abs(closed.seconds - 2.5) < 1e-9);
  ASSERT_TRUE(std::abs(closed.measure_seconds - 1.5) < 1e-9);
  ASSERT_EQ(closed.events, 1u);
  ASSERT_EQ(closed.measure_events, 1u);
  wait.update(true, 11.75, phase);
  wait.update(false, 12.0, phase);  // Worker clamps a late drain wake to issue end.
  closed = wait.snapshot(30.0, phase);
  ASSERT_TRUE(std::abs(closed.seconds - 2.75) < 1e-9);
  ASSERT_TRUE(std::abs(closed.measure_seconds - 1.75) < 1e-9);
  ASSERT_EQ(closed.events, 2u);
  ASSERT_EQ(closed.measure_events, 2u);
}

TEST(NativeLoadGeneratorPolicy, ClientIssueWaitRejectsClockBackAndSeparatesOrderedReasons) {
  using Reason = native_load::NativeSignedRunIssueHoldReason;
  native_load::IssueWaitTelemetry wait;
  native_load::MeasureWindow phase{10.0, 20.0};
  auto observe = [&](double at, std::uint64_t active, std::uint64_t source,
                     std::uint64_t canonical, std::uint64_t pacing,
                     std::uint32_t client, std::uint32_t query) {
    auto reason = native_load::native_signed_run_issue_hold_reason(
        16, active, source, canonical, true, pacing, client, query);
    wait.update(reason == Reason::client_capacity, at, phase);
    return reason;
  };
  ASSERT_TRUE(observe(10.0, 15, 16, 16, 16, 0, 0) == Reason::active_capacity);
  ASSERT_TRUE(observe(11.0, 16, 15, 16, 16, 0, 0) == Reason::source_capacity);
  ASSERT_TRUE(observe(12.0, 16, 16, 15, 16, 0, 0) == Reason::canonical_capacity);
  ASSERT_TRUE(observe(13.0, 16, 16, 16, 15, 0, 0) == Reason::pacing_credit);
  ASSERT_EQ(wait.snapshot(13.0, phase).events, 0u);
  ASSERT_TRUE(observe(14.0, 16, 16, 16, 16, 15, 0) == Reason::client_capacity);
  wait.update(false, 13.5, phase);  // Clock back cannot end a live interval.
  wait.update(false, std::numeric_limits<double>::quiet_NaN(), phase);
  wait.update(false, 14.5, {20.0, 10.0});
  ASSERT_TRUE(wait.active());
  ASSERT_TRUE(observe(15.0, 16, 16, 16, 16, 16, 15) == Reason::query_credit);
  ASSERT_TRUE(!wait.active());
  ASSERT_TRUE(observe(16.0, 16, 16, 16, 16, 16, 16) == Reason::none);
  auto result = wait.snapshot(20.0, phase);
  ASSERT_TRUE(std::abs(result.seconds - 1.0) < 1e-9);
  ASSERT_TRUE(std::abs(result.measure_seconds - 1.0) < 1e-9);
  ASSERT_EQ(result.events, 1u);
  ASSERT_EQ(result.measure_events, 1u);
}

TEST(NativeLoadGeneratorPolicy, PacingAndIssueWaitAggregationPreservesUnits) {
  native_load::PacingTelemetry first;
  native_load::PacingTelemetry second;
  native_load::MeasureWindow phase{10.0, 12.0};
  first.observe_refill(9.0, 13.0, 0.0, 100.0, 100.0, 100.0, phase);
  second.observe_refill(9.5, 12.5, 0.0, 100.0, 100.0, 100.0, phase);
  first.merge(second);
  ASSERT_TRUE(std::abs(first.clipped_tokens - 500.0) < 1e-9);
  ASSERT_TRUE(std::abs(first.measure_clipped_tokens - 350.0) < 1e-9);
  ASSERT_EQ(first.clipped_updates, 2u);
  ASSERT_EQ(first.measure_start.samples, 2u);
  ASSERT_EQ(first.measure_end.samples, 2u);
  ASSERT_TRUE(std::abs(first.measure_start.refilled_tokens - 150.0) < 1e-9);
  ASSERT_TRUE(std::abs(first.measure_end.refilled_tokens - 200.0) < 1e-9);
  ASSERT_TRUE(std::abs(first.measure_start.max_service_lag_seconds - 3.0) < 1e-9);
  ASSERT_TRUE(std::abs(first.update_gap.all_seconds - 4.0) < 1e-9);
  native_load::IssueWaitTelemetry worker_one;
  native_load::IssueWaitTelemetry worker_two;
  worker_one.update(true, 9.0, phase);
  worker_one.update(false, 11.0, phase);
  worker_two.update(true, 10.0, phase);
  worker_two.update(false, 12.0, phase);
  auto total = worker_one.snapshot(13.0, phase);
  total.merge(worker_two.snapshot(13.0, phase));
  ASSERT_TRUE(std::abs(total.seconds - 4.0) < 1e-9);
  ASSERT_TRUE(std::abs(total.measure_seconds - 3.0) < 1e-9);  // Worker-seconds, not two seconds of wall time.
  ASSERT_EQ(total.events, 2u);
  ASSERT_EQ(total.measure_events, 2u);
}
