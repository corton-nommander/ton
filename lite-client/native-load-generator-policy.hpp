...
// Telemetry uses the same monotonic clock as issuance. Measured durations are
// exact intersections with the half-open configured phase, not report deltas.
struct MeasureWindow {
  ...
  double overlap(double from, double to) const {
    if (!valid() || !std::isfinite(from) || !std::isfinite(to) || to <= from) {
      return 0.0;
    }
    return std::max(0.0, std::min(to, end) - std::max(from, begin));
  }
};

struct ObservedIntervalMax {
  double all_seconds{0.0};
  double measure_seconds{0.0};

  void observe(double from, double to, MeasureWindow window) {
    ...
    all_seconds = std::max(all_seconds, duration);
    measure_seconds = std::max(measure_seconds, window.overlap(from, to));
  }
  void merge(const ObservedIntervalMax& other) {
    all_seconds = std::max(all_seconds, other.all_seconds);
    measure_seconds = std::max(measure_seconds, other.measure_seconds);
  }
};

struct RefillBoundaryObservation {
  std::uint64_t samples{0};
  double refilled_tokens{0.0};
  double max_service_lag_seconds{0.0};

  void merge(const RefillBoundaryObservation& other) {
    samples += other.samples;
    refilled_tokens += other.refilled_tokens;
    max_service_lag_seconds = std::max(max_service_lag_seconds, other.max_service_lag_seconds);
  }
};

struct PacingTelemetry {
  double clipped_tokens{0.0};
  double measure_clipped_tokens{0.0};
  std::uint64_t clipped_updates{0};
  std::uint64_t measure_clipped_updates{0};
  ObservedIntervalMax update_gap;
  ObservedIntervalMax pump_gap;
  ObservedIntervalMax pump_duration;
  ObservedIntervalMax alarm_lateness;
  RefillBoundaryObservation measure_start;
  RefillBoundaryObservation measure_end;

  // Observe the existing endpoint-trapezoid refill without changing it. Lost
  // credit starts only when the bucket fills: uniformly prorating total loss
  // across the update interval would misattribute phase-boundary clipping.
  void observe_refill(double from, double to, double tokens_before,
                      double old_rate, double new_rate, double cap,
                      MeasureWindow window) {
    if (!std::isfinite(from) || !std::isfinite(to) || to <= from ||
        !std::isfinite(tokens_before) || tokens_before < 0.0 ||
        !std::isfinite(old_rate) || old_rate < 0.0 ||
        !std::isfinite(new_rate) || new_rate < 0.0 ||
        !std::isfinite(cap) || cap < 0.0) {
      return;
    }
    const auto duration = to - from;
    const auto earned = (old_rate + new_rate) * 0.5 * duration;
    if (!std::isfinite(earned) || !std::isfinite(tokens_before + earned)) {
      return;
    }
    update_gap.observe(from, to, window);
    const auto lost = std::max(0.0, tokens_before + earned - cap);
    clipped_tokens += lost;
    clipped_updates += lost > 0.0 ? 1 : 0;
    auto balance_at = [&](double at) {
      const auto fraction = std::clamp((at - from) / duration, 0.0, 1.0);
      const auto prefix_rate = old_rate + (new_rate - old_rate) * fraction;
      return tokens_before + (old_rate + prefix_rate) * 0.5 * duration * fraction;
    };
    if (!window.valid()) {
      return;
    }
    if (window.overlap(from, to) > 0.0 && lost > 0.0) {
      auto lost_by = [&](double at) {
        return std::clamp(balance_at(at) - cap, 0.0, lost);
      };
      const auto measured = std::max(0.0, lost_by(std::min(to, window.end)) -
                                            lost_by(std::max(from, window.begin)));
      measure_clipped_tokens += measured;
      measure_clipped_updates += measured > 0.0 ? 1 : 0;
    }
    auto boundary = [&](double at, RefillBoundaryObservation& observation) {
      if (observation.samples == 0 && from <= at && at <= to) {
        observation.samples = 1;
        // Counterfactual refilled balance at the exact boundary, before the
        // next issue turn. No issuance occurs inside this update interval.
        observation.refilled_tokens = std::min(cap, balance_at(at));
        observation.max_service_lag_seconds = to - at;
      }
    };
    boundary(window.begin, measure_start);
    boundary(window.end, measure_end);
  }

  void merge(const PacingTelemetry& other) {
    clipped_tokens += other.clipped_tokens;
    measure_clipped_tokens += other.measure_clipped_tokens;
    clipped_updates += other.clipped_updates;
    measure_clipped_updates += other.measure_clipped_updates;
    update_gap.merge(other.update_gap);
    pump_gap.merge(other.pump_gap);
    pump_duration.merge(other.pump_duration);
    alarm_lateness.merge(other.alarm_lateness);
    measure_start.merge(other.measure_start);
    measure_end.merge(other.measure_end);
  }
};

struct IssueWaitSnapshot {
  double seconds{0.0};
  double measure_seconds{0.0};
  std::uint64_t events{0};
  std::uint64_t measure_events{0};

  void merge(const IssueWaitSnapshot& other) {
    seconds += other.seconds;
    measure_seconds += other.measure_seconds;
    events += other.events;
    measure_events += other.measure_events;
  }
};

// One mutually exclusive observed issue-hold state, retained between callbacks.
// Taking a snapshot never commits an open interval or counts it twice.
class IssueWaitTelemetry {
 public:
  bool active() const { return active_; }

  void update(bool blocked, double now, MeasureWindow window) {
    if (!window.valid() || !std::isfinite(now) || (observed_ && now < last_at_)) {
      return;
    }
    observed_ = true;
    last_at_ = now;
    ...
    if (active_) {
      accumulate(closed_, started_at_, now, window);
    } else {
      started_at_ = now;
      ++closed_.events;
    }
  ...
  IssueWaitSnapshot snapshot(double now, MeasureWindow window) const {
    auto result = closed_;
    if (active_ && std::isfinite(now) && now >= last_at_) {
      accumulate(result, started_at_, now, window);
    }
    return result;
  }

 private:
  static void accumulate(IssueWaitSnapshot& into, double from, double to, MeasureWindow window) {
    into.seconds += std::max(0.0, to - from);
    const auto measured = window.overlap(from, to);
    into.measure_seconds += measured;
    into.measure_events += measured > 0.0 ? 1 : 0;
  }
  IssueWaitSnapshot closed_;
  bool active_{false};
