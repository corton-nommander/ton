...
void ExtMessagePool::update_last_masterchain_state(td::Ref<MasterchainState> state) {
  ...
  // A config rollback/reorg must not leave a physical message from the other
  // native wire mode at a source head. This is intentionally best-effort for
  // malformed test/bootstrap states; the admission paths turn a config parse
  // failure into a hard rejection before accepting new native work.
  if (!native_accounts_.empty()) {
    refresh_native_transfer_runs_mode_from_applied_state();
  }
}
...
td::Status ExtMessagePool::validate_native_admission_mode(const NativeAdmissionSnapshot &snapshot,
                                                           bool is_run) const {
  if (snapshot.runs_enabled == is_run) {
    return td::Status::OK();
  }
  if (snapshot.runs_enabled) {
    return td::Status::Error("scalar native transfers are disabled when source-signed runs are enabled");
  }
  return td::Status::Error("native transfer runs require global version 15 and capNativeTransferRuns");
}

void ExtMessagePool::update_native_transfer_runs_mode(bool enabled) {
  if (native_transfer_runs_mode_initialized_ && native_transfer_runs_mode_enabled_ == enabled) {
    return;
  }
  native_transfer_runs_mode_initialized_ = true;
  native_transfer_runs_mode_enabled_ = enabled;
...
void ExtMessagePool::purge_incompatible_native_messages(bool runs_enabled,
  ...
  for (const auto &[address, nonce, hash] : incompatible) {
    ...
    if (reservation_it->second.allow_broadcast_promise) {
      reservation_it->second.allow_broadcast_promise.set_error(td::Status::Error(reason));
    }
    reservation_it->second.insertion_failed(reason);
...
void ExtMessagePool::refresh_native_transfer_runs_mode_from_applied_state() {
  auto state = last_masterchain_state_;
  if (state.is_null()) {
    return;
  }
...
td::actor::Task<ExtMessagePool::CheckResult> ExtMessagePool::check_add_parsed_external_message_until(
  ...
  const auto native_tag = vm::load_cell_slice(message->root_cell()).prefetch_ulong(32);
  if (native_tag == block::NativeTransfer::magic || native_tag == block::NativeTransferRun::magic) {
    // Check the applied mode before raw-hash idempotence. Otherwise a scalar
    // NTFX (or a run after a rollback) already present in the pool could
    // bypass the current configuration gate through check_existing().
    auto snapshot = pin_native_admission_snapshot();
    if (snapshot.is_error()) {
      co_return snapshot.move_as_error();
    }
...
td::actor::Task<ExtMessagePool::BatchCheckResult> ExtMessagePool::check_add_external_messages_until(
  ...
  if (!native_indices.empty()) {
    auto snapshot = pin_native_admission_snapshot();
    if (snapshot.is_error()) {
      auto error = snapshot.move_as_error();
  ...
  while (callback->callback->cancellation_token.check().is_ok()) {
    ...
    if (!pending->empty()) {
      ...
      if (pending == &callback->pending_native && callback->callback->native_streaming) {
        batch_capacity = native_transport_publish_batch_capacity(*callback);
      }
      ...
      if (pending == &callback->pending_native && callback->callback->native_streaming &&
          callback->initial_native_publish_pending != 0) {
        CHECK(batch_size <= callback->initial_native_publish_pending);
        callback->initial_native_publish_pending -= batch_size;
      }
...
std::size_t ExtMessagePool::prefill_callback_native(const std::shared_ptr<InstalledCallback> &callback,
  ...
  if (count_install) {
    // The installation snapshot is the only native prefix allowed to cross
    // the actor queue as one complete physical transport-window push. Later
    // low-watermark work is always published in 512-message fragments.
    CHECK(callback->initial_native_publish_pending == 0);
    callback->initial_native_publish_pending = callback->pending_native.size();
  }
  return selected;
}
...
std::size_t ExtMessagePool::prefill_callback_native_low_watermark(
  ...
  const auto prefetch_capacity = native_transport_prefetch_capacity(*callback);
  if (callback->pending_native.size() >= prefetch_capacity) {
    return 0;
  }

  // The first look-ahead is the retained bounded producer fragment. A second
  // fragment is deliberately deferred until the Collator has completed one
  // full fragment of real consumption. This avoids eagerly pinning a second
  // callback-local prefix in candidates that are cancelled before doing work.
  std::size_t selected = 0;
  while (callback->pending_native.size() < prefetch_capacity) {
    const auto credit = native_transport_refill_credit(*callback);
    if (credit == 0) {
      break;
    }
    const auto requested = std::min({NATIVE_DELIVERY_CHUNK,
                                     prefetch_capacity - callback->pending_native.size(), credit});
    CHECK(requested != 0);
...
std::size_t ExtMessagePool::native_transport_prefetch_capacity(const InstalledCallback &callback) const {
  // The physical transport queue stays unchanged. Before one complete
  // scheduler fragment has been consumed, retain only the long-standing one
  // fragment producer look-ahead. After that proven progress, one additional
  // callback-local backup may overlap fair source selection with hand-off.
  return callback.callback->queue_state->native_consumed_count() >= NATIVE_DELIVERY_CHUNK
             ? NATIVE_TRANSPORT_LOW_WATERMARK_PREFETCH_CAPACITY
             : NATIVE_TRANSPORT_INITIAL_PREFETCH_CAPACITY;
}

std::size_t ExtMessagePool::native_transport_publish_batch_capacity(const InstalledCallback &callback) const {
  CHECK(callback.callback->native_streaming);
  if (callback.initial_native_publish_pending != 0) {
    // Preserve the known-good installation hand-off. `prefill_callback_native`
    // bounds this count by the configured physical transport window.
    return std::min({callback.callback->transport_message_capacity, callback.pending_native.size(),
                     callback.initial_native_publish_pending});
  }
  // Never repeat Cycle 58's 1,024-item blocked push: a later low-watermark
  // prefix is handed off one fair execution fragment at a time.
  return std::min(NATIVE_DELIVERY_CHUNK, callback.pending_native.size());
}
...
std::size_t ExtMessagePool::native_transport_selected_limit(const InstalledCallback &callback) const {
  ...
  // reserve at most two fair scheduler fragments while waiting for space, but
  // never another full candidate snapshot. The second fragment is separately
  // gated on one completed fragment in native_transport_prefetch_capacity().
  return callback.callback->transport_message_capacity + NATIVE_TRANSPORT_LOW_WATERMARK_PREFETCH_CAPACITY;
}
...
td::actor::Task<ExtMessagePool::CheckResult> ExtMessagePool::check_message(td::Ref<ExtMessage> message,
  ...
  if (native_tag == block::NativeTransfer::magic || native_tag == block::NativeTransferRun::magic) {
    // The account fetch above may come from a liteserver view. Native wire
    // mode, run locality and the signing domain instead come from the exact
    // locally applied masterchain snapshot used by batch admission.
    auto snapshot_result = pin_native_admission_snapshot();
    if (snapshot_result.is_error()) {
      co_return snapshot_result.move_as_error();
    }
    auto admission_snapshot = snapshot_result.move_as_ok();
    const bool is_run = native_tag == block::NativeTransferRun::magic;
    auto mode_status = validate_native_admission_mode(admission_snapshot, is_run);
    if (mode_status.is_error()) {
      co_return std::move(mode_status);
    }
    ...
    if (transfer) {
      signature_result =
          co_await td::actor::ask(verifier, &NativeSignatureVerifier::verify, transfer.value(),
                                  admission_snapshot.chain_domain)
              .wrap();
    } else {
      signature_result =
    ...
    auto current_snapshot = current_snapshot_result.move_as_ok();
    if (is_run && current_snapshot.block_id != admission_snapshot.block_id) {
