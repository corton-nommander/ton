...
#include "validator/impl/native-admission-config-cache.h"
...
TEST(NativeAdmissionBatch, ConfigCacheRequiresFullBlockAndStateIdentity) {
  using namespace ton;
  validator::NativeAdmissionConfigCache<int> cache;
  auto zero = Bits256::zero();
  auto one = zero;
  one.as_array()[31] = 1;
  BlockIdExt a{BlockId{masterchainId, shardIdAll, 42}, zero, zero};
  BlockIdExt fork{BlockId{masterchainId, shardIdAll, 42}, one, zero};
  BlockIdExt other_file{BlockId{masterchainId, shardIdAll, 42}, zero, one};
  BlockIdExt next{BlockId{masterchainId, shardIdAll, 43}, zero, zero};
  ASSERT_TRUE(cache.lookup(a, zero) == nullptr);
  cache.store(a, zero, 3);
  ASSERT_EQ(*cache.lookup(a, zero), 3);
  ASSERT_TRUE(cache.lookup(fork, zero) == nullptr);
  ASSERT_TRUE(cache.lookup(other_file, zero) == nullptr);
  ASSERT_TRUE(cache.lookup(next, zero) == nullptr);
  ASSERT_TRUE(cache.lookup(a, one) == nullptr);
  // Replacing a depth-3 projection with a new depth-2 state never leaves a
  // reusable old entry, including a different state root at the same block ID.
  cache.store(a, one, 2);
  ASSERT_EQ(*cache.lookup(a, one), 2);
  ASSERT_TRUE(cache.lookup(a, zero) == nullptr);
  cache.clear();
  ASSERT_TRUE(cache.lookup(a, one) == nullptr);
}
