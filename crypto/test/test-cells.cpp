...
using ProofTrace = std::vector<std::string>;

// A loaded-data proxy must keep its load/virtualize behavior even though it
// advertises the same level and hash as its concrete DataCell payload.
class ObservedProofCell final : public vm::Cell {
 public:
  ObservedProofCell(td::Ref<vm::Cell> payload, std::shared_ptr<ProofTrace> trace, bool fail_load)
      : payload_(std::move(payload)), trace_(std::move(trace)), fail_load_(fail_load) {}
  td::Status set_data_cell(td::Ref<vm::DataCell>&& data) const override {
    return payload_->set_data_cell(std::move(data));
  }
  td::Result<LoadedCell> load_cell() const override {
    trace_->push_back("load:" + payload_->get_hash().as_slice().str());
    if (fail_load_) {
      return td::Status::Error("intentional proof traversal fixture load failure");
    }
    loaded_ = true;
    return payload_->load_cell();
  }
  td::Ref<vm::Cell> virtualize(td::uint32 level) const override {
    trace_->push_back("virtualize:" + std::to_string(level));
    return vm::Cell::virtualize(level);
  }
  bool is_virtualized() const override { return payload_->is_virtualized(); }
  vm::CellUsageTree::NodePtr get_tree_node() const override { return payload_->get_tree_node(); }
  bool is_loaded() const override { return loaded_; }
  LevelMask get_level_mask() const override { return payload_->get_level_mask(); }
 private:
  td::uint16 do_get_depth(td::uint32 level) const override { return payload_->get_depth(level); }
  const Hash do_get_hash(td::uint32 level) const override { return payload_->get_hash(level); }
  td::Ref<vm::Cell> payload_;
  std::shared_ptr<ProofTrace> trace_;
  bool fail_load_;
  mutable bool loaded_{false};
};

td::Ref<vm::Cell> proof_fixture_cell(td::uint64 value, unsigned bits,
                                   std::initializer_list<td::Ref<vm::Cell>> children = {}) {
  vm::CellBuilder builder;
  CHECK(builder.store_ulong_rchk_bool(value, bits));
  for (const auto& child : children) {
    CHECK(builder.store_ref_bool(child));
  }
  return builder.finalize();
}

struct ProofStatFixture {
  std::shared_ptr<vm::CellUsageTree> usage = std::make_shared<vm::CellUsageTree>();
  std::shared_ptr<vm::CellUsageTree> foreign = std::make_shared<vm::CellUsageTree>();
  std::shared_ptr<ProofTrace> trace = std::make_shared<ProofTrace>();
  td::Ref<vm::Cell> root;

  explicit ProofStatFixture(unsigned kind) {
    auto callback = [events = trace](const vm::LoadedCell& loaded) {
      events->push_back("usage:" + loaded.data_cell->get_hash().as_slice().str() +
                        ":" + std::to_string(loaded.effective_level));
    };
    usage->set_cell_load_callback(callback);
    foreign->set_cell_load_callback(callback);
    auto leaf = proof_fixture_cell(7, 13);
    auto duplicate_hash = proof_fixture_cell(7, 13);
    CHECK(leaf.get() != duplicate_hash.get() && leaf->get_hash() == duplicate_hash->get_hash());
    auto left = proof_fixture_cell(3, 7, {leaf, duplicate_hash});
    auto right = proof_fixture_cell(5, 9, {leaf});
    auto dag = proof_fixture_cell(2, 5, {left, right});
    switch (kind) {
      case 0: break;
      case 1: root = proof_fixture_cell(0, 0); break;
      case 2: root = leaf; break;
      case 3: root = dag; break;
      case 4: root = vm::UsageCell::create(dag, usage->root_ptr()); break;
      case 5: root = proof_fixture_cell(6, 11, {vm::UsageCell::create(dag, usage->root_ptr()), leaf}); break;
      case 6: root = vm::UsageCell::create(dag, foreign->root_ptr()); break;
      case 7: root = proof_fixture_cell(6, 11, {vm::UsageCell::create(dag, foreign->root_ptr()), leaf}); break;
      case 8: {
        auto pruned = vm::CellBuilder::do_create_pruned_branch(dag, 1);
        root = vm::CellBuilder::create_merkle_proof(proof_fixture_cell(1, 1, {pruned, leaf}));
        break;
      }
      case 9: {
        auto pruned = vm::CellBuilder::do_create_pruned_branch(dag, 1);
        root = proof_fixture_cell(1, 1, {pruned, leaf})->virtualize(0);
        CHECK(root->is_virtualized());
        break;
      }
      case 10:
        root = vm::CellBuilder::create_merkle_update(vm::CellBuilder::do_create_pruned_branch(dag, 1), dag);
        break;
      case 11:
      case 12: {
        auto observed = td::Ref<ObservedProofCell>{true, dag, trace, kind == 12};
        root = proof_fixture_cell(3, 3, {std::move(observed), leaf});
        break;
      }
      default: UNREACHABLE();
    }
    trace->clear();  // Fixture construction is not part of traversal behavior.
  }
};

template <class Storage>
void add_proof_fixture(Storage& stat, const ProofStatFixture& fixture, unsigned mode) {
  if (mode == 0) {
    stat.add_cell(fixture.root);
  } else if (mode == 1) {
    stat.add_proof(fixture.root, fixture.usage.get());
  } else {
    stat.add_cell_and_proof(fixture.root, fixture.usage.get());
  }
}

void assert_proof_stat_equal(const vm::NewCellStorageStat& actual, const ReferenceCellStorageStat& expected) {
  ASSERT_EQ(actual.get_stat(), expected.get_stat());
  ASSERT_EQ(actual.get_proof_stat(), expected.get_proof_stat());
  ASSERT_EQ(actual.get_total_stat(), expected.get_total_stat());
}
}  // namespace

TEST(NativeProofStorageStat, ConcreteDataCellMatchesSliceTraversalAndCallbacks) {
  for (unsigned kind = 0; kind <= 12; ++kind) {
    for (unsigned mode = 0; mode < 3; ++mode) {
      ProofStatFixture actual_fixture{kind}, expected_fixture{kind};
      vm::NewCellStorageStat actual;
      ReferenceCellStorageStat expected;
      for (unsigned repetition = 0; repetition < 2; ++repetition) {
        add_proof_fixture(actual, actual_fixture, mode);
        add_proof_fixture(expected, expected_fixture, mode);
        assert_proof_stat_equal(actual, expected);
        ASSERT_EQ(*actual_fixture.trace, *expected_fixture.trace);
      }
      // Tentative accounting must consult the existing parent hash sets without
      // changing them. The trial shares the original graph and adds one leaf.
      auto make_trial = [](td::Ref<vm::Cell> shared) {
        auto fresh = proof_fixture_cell(1023, 17);
        return shared.not_null() ? proof_fixture_cell(9, 8, {std::move(shared), fresh})
                                 : proof_fixture_cell(9, 8, {fresh});
      };
      auto actual_root = make_trial(actual_fixture.root);
      auto expected_root = make_trial(expected_fixture.root);
      const auto before_stat = actual.get_stat();
      const auto before_proof = actual.get_proof_stat();
      ASSERT_EQ(actual.tentative_add_cell(actual_root), expected.tentative_add_cell(expected_root));
      ASSERT_EQ(actual.tentative_add_proof(actual_root, actual_fixture.usage.get()),
                expected.tentative_add_proof(expected_root, expected_fixture.usage.get()));
      ASSERT_EQ(actual.get_stat(), before_stat);
      ASSERT_EQ(actual.get_proof_stat(), before_proof);
      ASSERT_EQ(*actual_fixture.trace, *expected_fixture.trace);
      // Exact checkpoints replace their trial state, retaining the original
      // base. This is deliberately distinct from accumulating old roots.
      auto actual_trial = actual;
      auto expected_trial = expected;
      actual_trial.add_proof(actual_root, actual_fixture.usage.get());
      expected_trial.add_proof(expected_root, expected_fixture.usage.get());
      assert_proof_stat_equal(actual_trial, expected_trial);
      assert_proof_stat_equal(actual, expected);
      ASSERT_EQ(*actual_fixture.trace, *expected_fixture.trace);
    }
  }
...
TEST(NativeProofStorageStat, SharedDagKeepsReferenceMultiplicityAndUsageCutoff) {
  ProofStatFixture plain{3};
  vm::NewCellStorageStat stat;
  stat.add_cell_and_proof(plain.root, plain.usage.get());
  // Four unique hashes, but six incoming references including the root. The
  // separately allocated equal leaf and the third shared edge still count.
  const vm::NewCellStorageStat::Stat expected{4, 34, 6, 0};
  ASSERT_EQ(stat.get_stat(), expected);
  ASSERT_EQ(stat.get_proof_stat(), expected);
  stat.add_cell_and_proof(plain.root, plain.usage.get());
  ASSERT_EQ(stat.get_stat(), (vm::NewCellStorageStat::Stat{4, 34, 7, 0}));
  ASSERT_EQ(stat.get_proof_stat(), (vm::NewCellStorageStat::Stat{4, 34, 7, 0}));
  ProofStatFixture tracked{4};
  vm::NewCellStorageStat cutoff;
  cutoff.add_proof(tracked.root, tracked.usage.get());
  ASSERT_EQ(cutoff.get_proof_stat(), (vm::NewCellStorageStat::Stat{0, 0, 0, 1}));
  ASSERT_TRUE(tracked.trace->empty());
}
